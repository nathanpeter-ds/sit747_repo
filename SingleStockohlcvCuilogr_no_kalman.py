...
class SingleStockTraderohlcvCuilogr_no_kalman(gym.Env):
    def __init__(self, data, cash = 500000, transaction = 0.001, lookback = 30):
        ...
        self.High = self.data['High'].iloc[self.current_step]
        self.Low = self.data['Low'].iloc[self.current_step]
        self.Open = self.data['Open'].iloc[self.current_step]
        self.Volume = self.data['Volume'].iloc[self.current_step]
    ...
    def get_state(self):
        ...
        high_history = self.data['High'].iloc[self.current_step-self.lookback+1:self.current_step+1]
        low_history = self.data['Low'].iloc[self.current_step-self.lookback+1:self.current_step+1]
        open_history = self.data['Open'].iloc[self.current_step-self.lookback+1:self.current_step+1]
        vol_history = self.data['Volume'].iloc[self.current_step-self.lookback+1:self.current_step+1]
